...
export default function MetalPage() {
  const { type } = useParams(); // 'silver' or 'gold'
  ...
  const title = type === 'silver' ? 'Silver Jewelry' : 'Gold Jewelry';
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        // Fetch only collections where metal property strictly equals 'silver' or 'gold'
        ...
        const collectionIds = selectedCollections.map((c: any) => String(c.id));

        const allJewelries = await api.getJewelries();
        // Display only the individual jewelry products that belong to those filtered metal collections
        ...
        <div className="relative z-10 max-w-container-max mx-auto px-gutter w-full">
          <div className="max-w-2xl text-white">
            ...
            <p className="text-white/80 font-body-lg text-lg">
              Curated elegance tailored specifically for {title.toLowerCase()}.
            </p>
          </div>
        </div>
      </section>
