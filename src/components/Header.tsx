...
export default function Header() {
  ...
  const isSilver = location.pathname.includes('/metal/silver');
  const isGold = location.pathname.includes('/metal/gold') || location.pathname === '/';
    ...
    <div className="bg-white border-b border-outline-variant/30 py-2 relative z-40">
      <div className="max-w-container-max mx-auto px-gutter flex justify-center">
        <div className="flex w-full md:w-[600px] rounded-full border border-primary/40 p-1">
           <Link to="/metal/silver" className={`flex-1 text-center px-4 py-2 md:py-2.5 rounded-full text-sm md:text-base font-medium transition-colors ${isSilver ? 'bg-[#A32A4C] text-white' : 'text-on-surface hover:bg-[#A32A4C]/10'}`}>
              Silver Jewellery
           </Link>
           <Link to="/metal/gold" className={`flex-1 text-center px-4 py-2 md:py-2.5 rounded-full text-sm md:text-base font-medium transition-colors ${isGold ? 'bg-[#E5D38A] text-[#111111]' : 'text-on-surface hover:bg-[#E5D38A]/20'}`}>
              Gold Jewellery
           </Link>
        </div>
      </div>
